 ...
 */
public class RotateVerticalView extends View {
    ...
    public RotateVerticalView(Context context) {
        super(context);
        this.context = context;
        init();
    }

    public RotateVerticalView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        init();
    }

    public RotateVerticalView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        pin = BitmapFactory.decodeResource(context.getResources(),R.drawable.img_pointer);
        this.post(new Runnable() {
            ...
            public void run() {
                width = getWidth();
                height = getHeight();
                radius = 170.0f/290*pin.getIntrinsicHeight();
                centerX = width/2;
                centerY = 85.0f/290*pin.getIntrinsicHeight()+(height-pin.getIntrinsicHeight())/2;
                invalidate();
            }
        });
    }
